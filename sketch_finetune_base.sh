...
WEIGHT_FLAGS="--model_path pretrained/base_edge_decoder.pt --encoder_path coco-edge/coco-64-stage1_${task}/checkpoints/ema_0.999_080000.pt"
DIFFUSION_FLAGS=""
SAMPLE_FLAGS="--num_samples 2 --sample_c 1"
DATASET_FLAGS="--data_dir ./dataset/COCOSTUFF_train_${task}.txt --val_data_dir ./dataset/COCOSTUFF_val_${task}.txt --mode coco-edge"
mpiexec -n $NUM_GPUS --allow-run-as-root python ./image_train.py $MODEL_FLAGS $WEIGHT_FLAGS $TRAIN_FLAGS $SAMPLE_FLAGS $DIFFUSION_FLAGS  $DATASET_FLAGS
...
WEIGHT_FLAGS="--model_path pretrained/base_edge_decoder.pt --encoder_path coco-edge/coco-64-stage1-cont_${task}/checkpoints/ema_0.999_020000.pt"
DIFFUSION_FLAGS=""
